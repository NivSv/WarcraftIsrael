...
export class WowCharactersService {
  ...
  public async updateCharacterData(
    ...
    let wowCharacter = await this.prisma.wowCharacter.findFirst({
      where: { name: characterName, realm: realm },
    });
    if (wowCharacter == null) {
      wowCharacter = await this.prisma.wowCharacter.create({
        data: { name: characterName, realm: realm },
      });
    }
