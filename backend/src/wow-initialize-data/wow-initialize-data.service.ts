...
import { GUILDS, Guild_Member, MAX_LEVEL } from './wow-initialize-data.constants';
...
export class WowInitializeDataService {
    ...
    public async initializeAccessToken() {
        ...
        this.httpService.post("https://oauth.battle.net/token", {
            ...
            GUILDS.forEach(async (guild) => {
                await this.updateGuildInfo(guild.name, guild.realm, res.data.access_token);
            });
    ...
    public async updateGuildInfo(guildName: string, realm: string, blizardAccessToken: string) {
        //TODO: Add guild info to database
        //Adding guild members to database

        this.httpService.get(`https://eu.api.blizzard.com/data/wow/guild/${realm}/${guildName}/roster`,
            {
                params: {
                ...
                next: async (res) => {
                    ...
                    for (let member of members) {
                        if (member.character.level === MAX_LEVEL) {
                            await this.wowCharactersService.updateCharacterData(member.character.name.toLowerCase(), member.character.realm.slug.toLowerCase(), member.rank, blizardAccessToken);
                        }
                    }
